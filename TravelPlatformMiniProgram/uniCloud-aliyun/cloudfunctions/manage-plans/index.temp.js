...
exports.main = async (event, context) => {
	...
	async function updatePlanStatus(plan) {
		const now = Date.now();
		const newStatus = calculatePlanStatus(plan);
		
		// 如果状态发生变化且计划已经存在于数据库（有_id），则更新数据库
	...
	switch (event.action) {
		case 'create': {
			...
			const plan = {
				...
				status: calculatePlanStatus(event.plan) // 直接计算状态，不需要更新数据库
			};
		...
		case 'update': {
			...
			// 获取当前计划
			const currentPlan = await plansCollection.doc(event.plan_id).get();
			...
			// 检查权限
			if (currentPlan.data[0].user_id !== USERID) {
				return {
					code: 403,
					message: '无权操作此计划'
				};
			...
			// 更新计划
			await plansCollection.doc(event.plan_id).update({
				...plan,
		...
		case 'get': {
			...
			// 更新状态
			const planData = await updatePlanStatus(plan.data[0]);
		...
		case 'list': {
			...
			const updatedList = await Promise.all(
				listResult.data.map(plan => updatePlanStatus(plan))
			);
		...
		case 'public': {
			...
			// 更新每个计划的状态并关联用户信息
			const resultWithUserInfo = await Promise.all(
				listResult.data.map(async plan => {
